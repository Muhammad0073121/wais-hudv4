	...
	if not IsPauseMenuActive() then
		if inCar then
			if not belt then
				...
				exports.qbx_core:Notify(Config.Langs["belt-plug"], 'success')
				belt = true
			else
				TriggerEvent('InteractSound_CL:PlayOnOne', 'out', 0.5)
				exports.qbx_core:Notify(Config.Langs["belt-out"], 'error')
				belt = false
			end
		end
	end
	...
	if not IsPauseMenuActive() then
		if inCar then
			if not limitSet then
				exports.qbx_core:Notify(Config.Langs["dont-set-limit"])
			else
				...
				if GetPedInVehicleSeat(vehicle, -1) then
					...
					if not speedlimiter then
						if tonumber(speed) <= 20 then
							...
							exports.qbx_core:Notify(Config.Langs["limiter-error"])
						end
					else
						speedlimiter = false
...
RegisterCommand(Config.SpeedLimitCommand, function(x, args)
	if not tonumber(args[1]) then
		exports.qbx_core:Notify(Config.Langs["dont-enter-speed"])
		return
	end
